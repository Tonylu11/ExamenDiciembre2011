...
public class TestCuentas {
	 ...
	 * Persona Mortadelo.
	 */
	static Persona mortadelo;
	/**
	 * Persona Filem&oacute;n.
	 */
	static Persona filemon;

	public static void main(String[] args) {
		crearMortadelo();
		crearFilemon();
		System.out.println(mortadelo);
		System.out.println(filemon);
		reintegroMortadelo500();
		System.out.println("\n--Después de reintegro (500)\n" + mortadelo);
		mortadelo.setDireccion("13, Rue del Percebe");
		System.out.println("\n--Después de setDireccion(\"13, Rue del Percebe\")\n" + mortadelo.mostrar());
		transferenciaMortadeloAFilemon();
		System.out.println("\n--Después de la transferencia (500)\n" + mortadelo + "\n" + filemon);
		transferenciaMortadeloAFilemon();
		System.out.println("\n--Después de la transferencia (500)\n" + mortadelo + "\n" + filemon);
		reintegroMortadelo7();
		System.out.println("\n--Después de reintegro (7)\n" + mortadelo);
	}

	/**
	 * Retira de la cuenta de Mortadelo 7.
	 */
	private static void reintegroMortadelo7() {
		try {
			mortadelo.getCuenta().reintegro(7);
		} catch (NumerosRojosException e) {
			System.err.println("ERROR. No se ha podido realizar el reintegro.");
		}
	}

	/**
	 * Transfiere dinero de la cuenta de Mortadelo a la de Filem&oacute;n.
	 */
	private static void transferenciaMortadeloAFilemon() {
		try {
			mortadelo.getCuenta().transferencia(filemon.getCuenta(), 500);
		} catch (NumerosRojosException e) {
			System.err.println("ERROR. La cuenta no puede estar en números rojos.");
		} catch (TrasferenciaFallidaException e) {
			System.err.println("ERROR. No se ha podido realizar la transferencia.");
		}
	}

	/**
	 * Retira de la cuenta de Mortadelo 500.
	 */
	private static void reintegroMortadelo500() {
		try {
			mortadelo.getCuenta().reintegro(500);
		} catch (NumerosRojosException e) {
			System.err.println("ERROR. No se ha podido realizar el reintegro.");
		}
	}
	...
	/**
	 * Crea a Filem&oacute;n.
	 */
	private static void crearFilemon() {

		try {
			filemon = new Persona("Filemon", "55667788", "Calle del Medio", new Cuenta(2000));
		} catch (NumerosRojosException e) {
			System.err.println("ERROR. La cuenta no puede estar en números rojos.");
		}
	 ...
	 * Crea a Mortadelo.
	 */
	private static void crearMortadelo() {
		try {
			mortadelo = new Persona("Mortadelo", "11223344", "Mikasa", new Cuenta(1000));
		} catch (NumerosRojosException e) {
			System.err.println("ERROR. La cuenta no puede estar en números rojos.");
		}
		// } catch (DniInvalidoException e) {
